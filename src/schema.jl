...
"""
    StatsModels.@support_unprotect f sch_type=StatsModels.Schema

Generate methods necessary for function `f` to support [`unprotect`](@ref)
inside of a `@formula` with a schema of type `sch_type`.
...
"""
macro support_unprotect(op, sch_type=nothing)
    ex = quote
...
for op in SPECIALS, sch_type in (Schema, FullRank)
    @eval @support_unprotect $op $sch_type
end
