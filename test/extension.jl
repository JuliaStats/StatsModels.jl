...
@testset "Extended formula/models" begin
    ...
    @testset "Non-matrix term" begin
        ...
        @test f2.rhs isa Vector{AbstractTerm}
        @test f2.rhs == apply_schema(AbstractTerm[MatrixTerm(term(:x)), NonMatrixTerm(term(:y))], sch)
        @test modelcols(f2.rhs, d) == [hcat(d.x), d.y]

        # matrix term goes first
        f3 = apply_schema(f3, sch)
        @test f3.rhs isa Vector{AbstractTerm}
        @test f3.rhs == apply_schema(AbstractTerm[MatrixTerm(term(:y)), NonMatrixTerm(term(:x))], sch)
        @test modelcols(f3.rhs, d) == [hcat(d.y), d.x]

        f4 = apply_schema(f4, sch)
        @test all(t -> t isa NonMatrixTerm, f4.rhs)
        ...
        @test f5.rhs isa Vector{AbstractTerm}
        @test f5.rhs ==
            apply_schema(AbstractTerm[MatrixTerm(term.([:x, :y])), NonMatrixTerm(term(:y))], sch)
